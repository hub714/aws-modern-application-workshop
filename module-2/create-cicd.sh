...
aws cloudformation describe-stacks --stack-name "MythicalMysfitsCoreStack" --region us-east-2 | jq -r '[.Stacks[0].Outputs[] | {key: .OutputKey, value: .OutputValue}] | from_entries' > cfn-output.json
...
cp aws-cli/artifacts-bucket-policy.json aws-cli/artifacts-bucket-policy.json.original
cp aws-cli/code-build-project.json aws-cli/code-build-project.json.original
cp aws-cli/ecr-policy.json aws-cli/ecr-policy.json.original
cp aws-cli/code-pipeline.json aws-cli/code-pipeline.json.original
...
sed -i -e 's@REPLACE_ME_CODEPIPELINE_ROLE_ARN@'"${cfnOutputs[CodePipelineRole]}"'@' \
       -e 's@REPLACE_ME_ARTIFACTS_BUCKET_NAME@'"${cfnOutputs[MythicalArtifactBucket]}"'@' aws-cli/code-pipeline.json
